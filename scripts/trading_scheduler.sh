...
while true; do
    ...
    if [[ $weekday -le 5 ]]; then
        [[ $now == "0800" && -z $ran_pre_close   ]] && { ran_pre_close=1;   tlog "pre_close 시작";         { echo "[$(date '+%Y-%m-%d %H:%M:%S')] 시작"; /usr/bin/python3 "$DIR/orchestrator.py" mock pre_close         2>&1; echo "[$(date '+%Y-%m-%d %H:%M:%S')] 종료"; } >> "$DIR/logs/pre_close.log"         & }
        ...
        [[ $now == "1500" && -z $ran_entry       ]] && { ran_entry=1;        tlog "entry_monitor 시작";    { echo "[$(date '+%Y-%m-%d %H:%M:%S')] 시작"; /usr/bin/python3 "$DIR/orchestrator.py" mock entry_monitor     2>&1; echo "[$(date '+%Y-%m-%d %H:%M:%S')] 종료"; } >> "$DIR/logs/entry_monitor.log"     & }
    fi
